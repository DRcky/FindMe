...
export default function Profile({ auth, worker, stats, reviews }) {
    const { props } = usePage();
    ...
    {
        auth.user.id !== worker.user_id && (
            <button
                onClick={() => router.post(route('hires.store', worker.id))}
    ...
    const canReview = auth?.user?.id !== worker?.user_id; // evita reseñarse a sí mismo (front)
        ...
        <AuthenticatedLayout user={auth.user}>
            ...
            <div className="max-w-5xl mx-auto p-6 space-y-8">
                ...
                <div className="bg-white rounded shadow p-5">
                    ...
                    {worker.specialty && (
                        <p className="text-gray-700">Especialidad: <span className="font-medium">{worker.specialty.name}</span></p>
                    )}
                    {worker.user.city && (
                        <p className="text-gray-700">Ubicación: {worker.user.city}{worker.user.province ? `, ${worker.user.province}` : ''}</p>
                    )}
